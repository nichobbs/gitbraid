...
CI enforces coverage minimums (Linux only): **lines ≥70%, branches ≥70%, functions ≥75%** via `scripts/check-coverage.mjs`. These floors ratchet up as tests land; never lower them without a CHANGELOG note explaining why.
